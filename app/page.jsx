...
פרטיות: אין שמות, ת.ז., תאריכי לידה, כתובות. השתמש ב-[שם המטופל] וכו.
תרופות: אסור להמציא מינונים. לאימות בסוף כל שורת תרופה.
דגלים: הוסף סימן אזהרה בפתיח הסיכום אם יש סיכון.
מידע חסר: לבירור בלבד, לא להמציא.

פורמט:

התקבל במסגרת תורנות מיון.
הגיע עם הפנייה או ללא הפנייה.
מוכר או לא מוכר למוסדנו.
הגיע בליווי או בגפו.

רקע דמוגרפי - גיל, משפחה, מגורים, תעסוקה, ביטוח לאומי.

היסטוריה פסיכיאטרית ותרופות לאימות.
...
const MONOLOGUE_PROMPT = `אתה עוזר לפסיכיאטר מתמחה בישראל. כתוב מונולוג פורמלי ורהוט להצגה בעל פה לבכיר או כונן.

פרטיות: אין פרטים מזהים.
אורך: 8-12 משפטים. פסקה אחת רצופה, ללא כותרות.
מבנה: מי המטופל, למה הגיע, מה מתרשם, מה מתוכנן, מה ההתלבטויות.

התחל במילים הגיע מטופל וסיים במילים ורציתי להתייעץ בנוגע ל ואז ההתלבטות המרכזית.
טון פורמלי, בטוח, קליני.`;

const TREATMENT_PROMPT = `אתה פסיכיאטר בכיר מנוסה. כתוב תוכנית טיפול מעמיקה ויצירתית בשלושה צירים.

פרטיות: אין פרטים מזהים.
תרופות: סמן לאימות על כל המלצה. שיקולים ומשפחות בלבד, לא מינונים ספציפיים.

ציר תרופתי
היגיון קליני לפי המקרה הספציפי.
המלצות עם סימון לאימות, מנגנון, יעילות, שיקולי בטיחות, ניטור.
PRN אם רלוונטי.

ציר סוציאלי
הערכת המצב הסוציאלי.
ביטוח לאומי וזכויות ספציפיות.
סל שיקום - אפשרויות כמו דיור, תעסוקה, לימודים, מועדון.
מעורבות משפחתית.
המשך טיפול - מרפאה, מסגרת, עמותות.

ציר שיחתי ופסיכותרפויטי
גישה מומלצת עם נימוק - CBT או DBT או פסיכודינמי או EMDR או Schema או IPT או MBCT.
פוקוס - מה לעבד.
    ...
    <div dir="rtl" style={{ fontFamily: "Heebo, system-ui, sans-serif", minHeight: '100vh', background: '#F7F6F3' }}>
      <style>{`
        ...
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}body{margin:0}
        .out{white-space:pre-wrap;line-height:1.9;font-size:15px;color:#2C2C2C}
        ...
        textarea:focus,button:focus{outline:none}button{cursor:pointer}
      `}</style>
        ...
        {stage === 'input' && (
          <div className="fade">
            ...
            <div style={{ background: '#EFF6FF', borderRadius: 14, padding: '10px 14px', marginBottom: 16, display: 'flex', gap: 10 }}>
              ...
              <span style={{ fontSize: 13, color: '#1E40AF', lineHeight: 1.5 }}>iPhone: לחץ על תיבת הטקסט, לחץ על המיקרופון ליד מקש הרווח, ודבר.</span>
            </div>
